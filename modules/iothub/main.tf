...
resource "azurerm_template_deployment" "two" {
  name                = "iothub-deployment2"
  resource_group_name = var.resource_group_name
  parameters = {
    resourceGrp           = var.resource_group_name
    subscriptd            = var.azure_subscription_id
    iotHubName            = azurerm_iothub.main.name
    eventHubNameSpace     = var.evth1ns
    eventHubName          = "salestransactions"
    eventSubscriptionName = "salestransactions"
    filterMessageType     = "fuel-sale-transaction"
  }
  template_body   = <<DEPLOY
{
  "$schema": "https://schema.management.azure.com/schemas/2019-04-01/deploymentTemplate.json#",
...
resource "azurerm_template_deployment" "third3" {
  ...
  parameters = {
    ...
    iotHubName            = azurerm_iothub.main.name
    eventHubNameSpace     = var.evth1ns
    eventHubName          = "atgconfigs"
    eventSubscriptionName = "atgconfigs"
    filterMessageType     = "atg-tank-configuration"
  }
  template_body   = <<DEPLOY
{
  "$schema": "https://schema.management.azure.com/schemas/2019-04-01/deploymentTemplate.json#",
...
resource "azurerm_template_deployment" "fourth4" {
  ...
  parameters = {
    ...
    iotHubName            = azurerm_iothub.main.name
    eventHubNameSpace     = var.evth1ns
    eventHubName          = "nozzleconfigs"
    eventSubscriptionName = "nozzleconfigs"
    filterMessageType     = "nozzle-configuration"
  }
  template_body   = <<DEPLOY
{
  "$schema": "https://schema.management.azure.com/schemas/2019-04-01/deploymentTemplate.json#",
